        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det): # 假如预测到有目标（盒子存在）
                ...
                for *xyxy, conf, cls in det: # x1 y1 x2 y2 cls class
                    ...
                    t = 0
                    for i, each in enumerate(list):
                        # X = np.vstack([features[each], _feature])
                        # t = pdist(X, 'hamming')
            ...
            if save_img: # 保存图片or视频
                if dataset.mode == 'images':
                    ...
                    if (cv2.waitKey(30)) != 27:
                        cv2.imshow("Face Recognition", im0)
                    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', type=str, default='inference/output/videos', help='output folder')  # output folder
    ...
    parser.add_argument('--open_rf', default=False, help='if open real/fake 1 0 ')
